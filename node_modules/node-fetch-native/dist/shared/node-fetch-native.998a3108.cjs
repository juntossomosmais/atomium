...
const pipeline = node_util.promisify(Stream__default.pipeline);
...
class Body {
	constructor(body, {
		...
		} else if (body instanceof Stream__default) ; else if (body instanceof FormData) {
			// Body is FormData
		...
		if (node_buffer.Buffer.isBuffer(body)) {
			stream = Stream__default.Readable.from(body);
		} else if (isBlob(body)) {
			stream = Stream__default.Readable.from(body.stream());
		}
...
/* c8 ignore next 9 */
const validateHeaderName = typeof http__default.validateHeaderName === 'function' ?
...
/* c8 ignore next 9 */
const validateHeaderValue = typeof http__default.validateHeaderValue === 'function' ?
	...
	return new Promise((resolve, reject) => {
		...
		request_.on('response', response_ => {
			...
			if (isRedirect(response_.statusCode)) {
				...
				switch (request.redirect) {
					...
					case 'follow': {
						...
						if (response_.statusCode !== 303 && request.body && options_.body instanceof Stream__default.Readable) {
			...
			if (codings === 'gzip' || codings === 'x-gzip') {
				body = Stream.pipeline(body, zlib__default.createGunzip(zlibOptions), error => {
					if (error) {
						reject(error);
					}
				});
			...
			if (codings === 'deflate' || codings === 'x-deflate') {
				...
				raw.once('data', chunk => {
					...
					if ((chunk[0] & 0x0F) === 0x08) {
						body = Stream.pipeline(body, zlib__default.createInflate(), error => {
							if (error) {
								reject(error);
							}
						});
					} else {
						body = Stream.pipeline(body, zlib__default.createInflateRaw(), error => {
							if (error) {
								reject(error);
							}
						});
			...
			// For br
			if (codings === 'br') {
				body = Stream.pipeline(body, zlib__default.createBrotliDecompress(), error => {
					if (error) {
						reject(error);
					}
				});
