...
function findNodeMatchingVersion(graph, packageName, versionExpr) {
    ...
    const nodes = Object.values(graph.externalNodes).filter((n) => n.data.packageName === packageName);
...
function switchNodeToHoisted(node, builder) {
    ...
    sources.forEach((source) => builder.addExternalNodeDependency(source, node.name));
}
