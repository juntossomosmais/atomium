...
function addNodes(data, builder, keyMap) {
    ...
    if (data.lockfileVersion > 1) {
        Object.entries(data.packages).forEach(([path, snapshot]) => {
            ...
            const node = createNode(packageName, version, path, nodes, keyMap, !path.includes('/node_modules/'));
            if (node) {
...
function addV1Node(packageName, snapshot, path, nodes, keyMap, builder) {
    const node = createNode(packageName, snapshot.version, path, nodes, keyMap, !path.includes('/node_modules/'));
    if (node) {
        builder.addExternalNode(node);
    }
...
function createNode(packageName, version, key, nodes, keyMap, isHoisted) {
    var _a;
        ...
        invertedGraph.dependencies[node.name].forEach(({ target }) => {
            ...
            if (visitedNodes.has(targetNode)) {
                visitedNodes.get(targetNode).forEach((path) => {
                    const parentPath = findParentPath(path, node.data.packageName, visitedPaths) + '/';
                    const mappedPackage = mapPackage(rootLockFile, node.data.packageName, node.data.version, parentPath);
...
function findParentPath(path, packageName, visitedPaths) {
