...
exports.getLogger = exports.wrapAngularDevkitSchematic = exports.mockSchematicsForTesting = exports.overrideCollectionResolutionForTesting = exports.runMigration = exports.generate = exports.NxScopeHostUsedForWrappedSchematics = exports.NxScopedHost = exports.scheduleTarget = void 0;
...
class NxScopedHost extends core_1.virtualFs.ScopedHost {
    ...
    read(path) {
        if (path === 'angular.json' || path === '/angular.json') {
            return this.readMergedProjectConfiguration().pipe((0, operators_1.map)((r) => Buffer.from(JSON.stringify((0, angular_json_1.toOldFormat)(r)))));
        }
        else {
            return super.read(path);
        }
    }
    readMergedProjectConfiguration() {
        return (0, rxjs_1.zip)((0, rxjs_1.from)((0, project_graph_1.createProjectGraphAsync)()), this.readExistingAngularJson()).pipe((0, operators_1.concatMap)((arg) => {
            const graph = arg[0];
            const ret = (arg[1] || { projects: {} });
            const projectJsonReads = [];
            for (let projectName of Object.keys(graph.nodes)) {
                if (!ret.projects[projectName]) {
                    projectJsonReads.push(this.readExistingProjectJson((0, path_1.join)(graph.nodes[projectName].data.root, 'project.json')));
                }
            }
            return (0, rxjs_1.zip)(...projectJsonReads).pipe((0, operators_1.map)((projectJsons) => {
                projectJsons
                    .filter((p) => p !== null)
                    .forEach((p) => {
                    delete p.version;
                    ret.projects[p.name] = Object.assign(Object.assign({}, p), { root: graph.nodes[p.name].data.root });
                });
                return ret;
            }));
    ...
    write(path, content) {
        if (path === 'angular.json' || path === '/angular.json') {
            ...
            return (0, rxjs_1.zip)(this.readMergedProjectConfiguration(), this.readExistingAngularJson()).pipe((0, operators_1.concatMap)((arg) => {
                        ...
                        (0, project_configuration_1.updateProjectConfiguration)({
                            ...
                            write: (path, content) => {
                                if (existingConfig.projects[projectName]) {
                                    const updatedContent = this.mergeProjectConfiguration(existingConfig.projects[projectName], projects[projectName]);
                                    if (updatedContent) {
    ...
    mergeProjectConfiguration(existing, updated) {
    ...
    readExistingProjectJson(path) {
        return super
            .exists(path)
            .pipe((0, operators_1.concatMap)((r) => r
            ? super
                .read(path)
                .pipe((0, operators_1.map)((r) => (0, angular_json_1.toNewFormat)((0, json_2.parseJson)(arrayBufferToString(r)))))
            : (0, rxjs_1.of)(null)));
    }
}
