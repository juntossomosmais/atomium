...
function resolve(source, file, options) {
  ...
  if (!cachedOptions || previousOptionsHash !== (optionsHash = digestHashObject(options))) {
    ...
    cachedOptions = __spreadProps(__spreadValues({}, options), {
      ...
      fileSystem: new enhancedResolve__default["default"].CachedInputFileSystem(
        fileSystem,
        5 * 1e3
      ),
  ...
  if (!resolver || resolverCachedOptions !== cachedOptions) {
    resolver = enhancedResolve__default["default"].ResolverFactory.createResolver(cachedOptions);
...
function getMappedPath(source, file, extensions = defaultExtensions, retry) {
  ...
  if (RELATIVE_PATH_PATTERN.test(source)) {
    ...
    paths = mappers.map(
      ...
      try {
        const stat = fs__default["default"].statSync(mappedPath, { throwIfNoEntry: false });
        if (stat === void 0)
          return false;
        if (stat.isFile())
          return true;
