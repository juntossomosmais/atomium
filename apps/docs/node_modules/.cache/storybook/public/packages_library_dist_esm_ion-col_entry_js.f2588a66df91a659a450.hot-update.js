"use strict";
globalThis["webpackHotUpdateatomium_docs"]("packages_library_dist_esm_ion-col_entry_js",{
...
const Col = class {
  ...
  calculateSize() {
    ...
    const colSize = columns === 'auto' ? 'auto' : SUPPORTS_VARS ? `calc(calc(${columns} / var(--ion-grid-columns, 12)) * 100%)` : columns / 12 * 100 + '%';
    return {
      flex: `0 0 ${colSize}`,
      width: `${colSize}`,
      'max-width': `${colSize}`
  ...
  calculatePosition(property, modifier) {
    ...
    const amount = SUPPORTS_VARS ? `calc(calc(${columns} / var(--ion-grid-columns, 12)) * 100%)` : columns > 0 && columns < 12 ? columns / 12 * 100 + '%' : 'auto';
    return {
      [modifier]: amount
    };
  }
...
//# sourceMappingURL=packages_library_dist_esm_ion-col_entry_js.f2588a66df91a659a450.hot-update.js.map