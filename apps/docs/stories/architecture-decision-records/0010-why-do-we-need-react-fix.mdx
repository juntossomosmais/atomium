...
> ⚠️ **Superseded by [2025-11-04: React Output Target Update](./2025-11-04-react-output-target-update.mdx).** The `reactBooleanFixOutputTarget` patch was removed when we moved to `@stencil/react-output-target@^1.5.3`, whose runtime (`@lit/react`) sets DOM properties instead of stringified attributes, so the boolean bug described here no longer occurs. This ADR is kept for historical context.
